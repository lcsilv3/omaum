...
class TestAutomationManager:
    ...
    def install_dependencies(self):
        ...
        if (self.project_root / 'requirements-dev.txt').exists():
            subprocess.run([
                str(self.pip_exe), 'install', '-r', 'requirements-dev.txt'
            ], check=True)
    ...
    def run_automated_tests(self):
        ...
        if test_script.exists():
            result = subprocess.run([
                str(self.python_exe), str(test_script)
            ], capture_output=True, text=True)
            
            print(f"📊 Resultado dos testes: {'✅ SUCESSO' if result.returncode == 0 else '❌ FALHA'}")
    ...
    def run_parallel_tests(self):
        ...
        if parallel_script.exists():
            ...
            print(f"📊 Resultado dos testes paralelos: {'✅ SUCESSO' if result.returncode == 0 else '❌ FALHA'}")
...
def main():
    ...
    parser.add_argument('--tests-only', action='store_true', help='Apenas executar testes')
    parser.add_argument('--coverage-only', action='store_true', help='Apenas análise de cobertura')
