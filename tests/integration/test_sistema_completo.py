...
class FluxoCompletoSistemaTest(TransactionTestCase):
    ...
    def test_fluxo_completo_aluno_matricula_turma_presenca(self):
        ...
        response = self.client.post(
            reverse("alunos:api_criar"), aluno_data, content_type="application/json"
        )
    ...
    def test_performance_sistema_completo(self):
        ...
        alunos_com_matriculas = (
            Aluno.objects.select_related("tipo_aluno")
            .prefetch_related("matricula_set__curso")
            .all()
        )

        # Consulta 2: Todas as turmas com seus alunos
        turmas_com_alunos = (
            Turma.objects.select_related("curso", "status")
...
class APIIntegrationTest(TestCase):
    ...
    def test_api_crud_completo_aluno(self):
        ...
        # UPDATE
        update_data = {"nome": "Maria Santos Silva", "email": "maria.silva@example.com"}
    ...
    def test_api_relacionamentos_complexos(self):
        ...
        matricula_data = {"aluno": aluno.id, "curso": curso.id, "valor_pago": "200.00"}
    ...
    def test_api_filtros_e_busca(self):
        ...
        response = self.client.get(reverse("alunos:api_buscar"), {"q": "Silva"})
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 1
        ...
        response = self.client.get(reverse("alunos:api_lista"), {"ativo": "true"})
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 2  # aluno1 e aluno3

        # Teste de paginação
        response = self.client.get(reverse("alunos:api_lista"), {"page": 1, "limit": 2})
        assert response.status_code == 200
        data = response.json()
...
class ConcorrenciaTest(TransactionTestCase):
    ...
    def test_presenca_concorrente(self):
            ...
            try:
                with transaction.atomic():
                    presenca = PresencaFactory(
                        aluno=aluno, turma=turma, data_aula=data_aula
                    )
        ...
        presenca = Presenca.objects.get(aluno=aluno, turma=turma, data_aula=data_aula)
        assert presenca.aluno == aluno
        assert presenca.turma == turma
...
class IntegrationWorkflowTest:
    ...
    def test_workflow_completo_escola(self):
        ...
        tipo_curso_tecnico = TipoCurso.objects.create(
            nome="Técnico", descricao="Curso técnico"
        )
        status_matricula_ativa = StatusMatricula.objects.create(
            nome="Ativa", descricao="Matrícula ativa"
        )
        status_turma_andamento = StatusTurma.objects.create(
            nome="Em Andamento", descricao="Turma em andamento"
        )
        status_presenca_presente = StatusPresenca.objects.create(
            nome="Presente", descricao="Aluno presente"
        )
