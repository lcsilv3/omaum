...
class AtividadeViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        if serializer.is_valid():
            try:
                atividade = AtividadeService.criar_atividade(serializer.validated_data)
    ...
    def update(self, request, pk=None):
        ...
        if serializer.is_valid():
            try:
                atividade_atualizada = AtividadeService.atualizar_atividade(
                    pk, serializer.validated_data
                )
                if atividade_atualizada:
                    response_serializer = self.get_serializer(atividade_atualizada)
    ...
    def destroy(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {"error": "Atividade não encontrada"}, status=status.HTTP_404_NOT_FOUND
            )
        except Exception as e:
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def por_turma(self, request):
        ...
        if not turma_id:
            return Response(
                {"error": "turma_id é obrigatório"}, status=status.HTTP_400_BAD_REQUEST
            )

        atividades = AtividadeService.listar_atividades_por_turma(turma_id)
        serializer = self.get_serializer(atividades, many=True)
        return Response(serializer.data)
    ...
    def por_curso(self, request):
        ...
        if not curso_id:
            return Response(
                {"error": "curso_id é obrigatório"}, status=status.HTTP_400_BAD_REQUEST
            )

        atividades = AtividadeService.listar_atividades_por_curso(curso_id)
        serializer = self.get_serializer(atividades, many=True)
        return Response(serializer.data)
