...
def listar_atividades_ritualisticas(request):
    ...
    try:
        ...
        # Consulta base
        atividades = AtividadeRitualistica.objects.all().prefetch_related(
            "participantes"
        )
        ...
        logger.error(
            f"Erro ao listar atividades ritualísticas: {str(e)}", exc_info=True
        )
        messages.error(request, f"Ocorreu um erro ao listar as atividades: {str(e)}")
        return render(
            request,
            "atividades/ritualisticas/listar_atividades_ritualisticas.html",
            {
...
def editar_atividade_ritualistica(request, id):
    ...
    try:
        ...
        logger.error(
            f"Erro ao editar atividade ritualística {id}: {str(e)}", exc_info=True
        )
...
def detalhar_atividade_ritualistica(request, id):
    ...
    try:
        ...
        atividade = get_object_or_404(
            AtividadeRitualistica.objects.prefetch_related("participantes"), id=id
        )
        ...
        logger.error(
            f"Erro ao detalhar atividade ritualística {id}: {str(e)}", exc_info=True
        )
        messages.error(
...
def excluir_atividade_ritualistica(request, id):
    ...
    try:
        ...
        logger.error(
            f"Erro ao excluir atividade ritualística {id}: {str(e)}", exc_info=True
        )
...
def copiar_atividade_ritualistica(request, id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "atividades:detalhar_atividade_ritualistica", pk=nova_atividade.id
            )
        else:
