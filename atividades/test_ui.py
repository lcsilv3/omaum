...
class AtividadesUITest(StaticLiveServerTestCase):
    ...
    def test_fluxo_completo_usuario(self):
        ...
        header = None
        try:
            header = self.driver.find_element(By.XPATH, "//h1[contains(text(), 'Atividade')]")
        except Exception:
            try:
                header = self.driver.find_element(By.XPATH, "//*[self::h2 or self::h3][contains(text(), 'Atividade')]")
            except Exception:
        ...
        self.driver.find_element(By.NAME, 'descricao').send_keys('Atividade criada via Selenium')
