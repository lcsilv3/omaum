...
totais_existentes = TotalAtividadeMes.objects.filter(
    turma=turma, ano=ano, mes=mes
)
print(f"Totais já registrados: {totais_existentes.count()}")
for total in totais_existentes:
    print(f"  - Atividade: {total.atividade.nome} (ID: {total.atividade.id}), Qtd: {total.qtd_ativ_mes}")
