...
def popular_codigos():
    with open(CSV_PATH, encoding='latin1') as csvfile:
        ...
        for row in reader:
            ...
            descricao = row.get('Descrição') or row.get('descricao') or row.get('Descri��o')
            ...
            for t in TipoCodigo.objects.all():
                if normaliza(t.nome) == nome_tipo_normalizado:
                    ...
                    break
            if not tipo:
                print(f"Tipo não encontrado: {nome_tipo} (normalizado: {nome_tipo_normalizado})")
                continue
