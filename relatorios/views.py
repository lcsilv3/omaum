...
def relatorio_historico(request):
    ...
    # Filtrar registros
    registros = RegistroHistorico.objects.select_related(
        "aluno", "codigo", "codigo__tipo_codigo"
    ).all()
...
def relatorio_historico_pdf(request):
    ...
    context = {
        ...
        "data_geracao": timezone.now(),
        "aluno": Aluno.objects.get(id=aluno_id) if aluno_id else None,
        "tipo_codigo": TipoCodigo.objects.get(id=tipo_codigo_id)
        if tipo_codigo_id
        else None,
    ...
    try:
        ...
        response = HttpResponse(content_type="application/pdf")
        response["Content-Disposition"] = (
            'attachment; filename="relatorio_historico.pdf"'
        )
        pdf_status = pisa.CreatePDF(html, dest=response)
        if pdf_status.err:
            return HttpResponse("Erro ao gerar PDF", status=500)
...
def relatorio_atividades_pdf(request):
    ...
    context = {
        "atividades": atividades.order_by("-data_inicio"),
        "data_geracao": timezone.now(),
        "tipo_selecionado": dict(AtividadeAcademica.TIPO_CHOICES).get(
            tipo_atividade, ""
        ),
    ...
    try:
        ...
        response = HttpResponse(content_type="application/pdf")
        response["Content-Disposition"] = (
            'attachment; filename="relatorio_atividades.pdf"'
        )
