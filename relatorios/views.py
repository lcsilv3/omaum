...
def relatorio_punicoes(request):
    ...
    # Obter listas para os filtros
    alunos = Aluno.objects.all()
    tipos_punicao = Punicao.objects.values_list(
        "tipo_punicao", flat=True
    ).distinct()
...
def relatorio_punicoes_pdf(request):
    ...
    # Criar dados da tabela
    data = [
        ["Aluno", "Tipo de Punição", "Data", "Descrição"]
    ]  # Linha de cabeçalho
    for punicao in punicoes:
        data.append(
            [
                ...
                (
                    punicao.descricao[:100] + "..."
                    if len(punicao.descricao) > 100
                    else punicao.descricao
                ),
            ]
        )
