...
class RelatoriosUITest(StaticLiveServerTestCase):
    ...
    def test_fluxo_completo_usuario(self):
        ...
        self.assertTrue(
            any(txt in body for txt in ["Sair", "Administração", "Logout"]),
        ...
        try:
            header = self.driver.find_element(
                By.XPATH, "//h1[contains(text(), 'Relatório')]"
            )
        except Exception:
            try:
                header = self.driver.find_element(
                    By.XPATH, "//*[self::h2 or self::h3][contains(text(), 'Relatório')]"
                )
            except Exception:
                body = self.driver.find_element(By.TAG_NAME, "body").text
                self.assertIn("Relatório", body)
        if header:
            self.assertIn("Relatório", header.text)
        ...
        self.driver.get(f"{self.live_server_url}/relatorios/")
        body = self.driver.find_element(By.TAG_NAME, "body").text
        self.assertIn("Relatório Selenium", body)
        self.driver.find_element(By.LINK_TEXT, "Relatório Selenium").click()
        time.sleep(1)
        self.assertIn("Relatório Selenium", self.driver.page_source)
        ...
        self.driver.get(f"{self.live_server_url}/relatorios/")
        body = self.driver.find_element(By.TAG_NAME, "body").text
        self.assertNotIn("Relatório Selenium Editado", body)
