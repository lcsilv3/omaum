...
class RelatorioSerializer(serializers.ModelSerializer):
    ...
    def validate_titulo(self, value):
        ...
        if not value or not value.strip():
            raise serializers.ValidationError("O título do relatório é obrigatório")
        return value.strip()

    def validate_conteudo(self, value):
        """Valida o conteúdo do relatório."""
        if not value or not value.strip():
            raise serializers.ValidationError("O conteúdo do relatório é obrigatório")
        return value.strip()
...
class RelatorioCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_titulo(self, value):
        ...
        if Relatorio.objects.filter(titulo=value.strip()).exists():
            raise serializers.ValidationError("Já existe um relatório com este título")
...
class RelatorioUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Relatorio
        fields = ["titulo", "conteudo"]
        extra_kwargs = {"titulo": {"required": False}, "conteudo": {"required": False}}
    ...
    def validate_titulo(self, value):
        ...
        if value is not None:
            ...
            if (
                instance
                and Relatorio.objects.filter(titulo=value.strip())
