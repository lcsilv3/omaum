    ...
    with transaction.atomic():
        ...
        for presenca in presencas_sem_turma:
            if hasattr(presenca.atividade, 'turmas') and presenca.atividade.turmas.exists():
                ...
                presenca.save()
                corrigidas_turma += 1
                print(f"  ✓ Presença ID {presenca.id} - turma definida: {primeira_turma.nome}")
        ...
        for presenca in presencas_academicas:
            ...
            if presenca.turma and not presenca.atividade:
                ...
                atividade_proxima = Atividade.objects.filter(
                    turmas=presenca.turma,
                    data_inicio=presenca.data
                ).first()
            ...
            if mudou:
                ...
                print(f"  ✓ PresencaAcademica ID {presenca.id} - relacionamentos corrigidos")
        
        print(f"📊 {corrigidas_academicas} presenças acadêmicas tiveram relacionamentos corrigidos")
