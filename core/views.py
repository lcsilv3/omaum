...
def registro_usuario(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, "Conta criada com sucesso! Agora você pode fazer login."
            )
...
def painel_controle(request):
    ...
    if not request.user.is_staff:
        messages.error(
            request, "Você não tem permissão para acessar o painel de controle."
        )
    ...
    try:
        ...
        messages.error(
            request, f"Ocorreu um erro ao carregar o painel de controle: {str(e)}"
        )
...
def atualizar_configuracao(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            config.mensagem_manutencao = request.POST.get(
