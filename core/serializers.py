...
class ConfiguracaoSistemaSerializer(serializers.ModelSerializer):
    ...
    def validate_chave(self, value):
        ...
        if not value or not value.strip():
            raise serializers.ValidationError("A chave da configuração é obrigatória")
        return value.strip()

    def validate_valor(self, value):
        """Valida o valor da configuração."""
        if not value or not value.strip():
            raise serializers.ValidationError("O valor da configuração é obrigatório")
        return value.strip()
...
class LogAtividadeSerializer(serializers.ModelSerializer):
    ...
    def validate_acao(self, value):
        ...
        if not value or not value.strip():
            raise serializers.ValidationError("A ação é obrigatória")
        return value.strip()
