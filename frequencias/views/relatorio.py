...
def relatorio_frequencias(request):
    ...
    try:
        ...
        # Percentuais
        percentual_liberados = (
            (total_liberados / total_carencias * 100) if total_carencias > 0 else 0
        )
        percentual_nao_liberados = (
            (total_nao_liberados / total_carencias * 100) if total_carencias > 0 else 0
        )
        ...
        frequencias_por_mes = (
            FrequenciaMensal.objects.values("mes", "ano")
...
def exportar_frequencia_csv(request, frequencia_id):
    ...
    try:
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="frequencia_{frequencia.turma.nome}_{frequencia.get_mes_display()}_{frequencia.ano}.csv"'
        )
...
def historico_frequencia(request, aluno_cpf):
    ...
    try:
        ...
        carencias = Carencia.objects.filter(aluno=aluno).select_related(
            "frequencia_mensal", "frequencia_mensal__turma"
        )
        ...
        carencias = carencias.order_by(
            "-frequencia_mensal__ano", "-frequencia_mensal__mes"
        )
        ...
        anos = (
            FrequenciaMensal.objects.filter(carencia__aluno=aluno)
        ...
        context = {
            ...
            "filtros": {"curso": curso_id, "turma": turma_id, "periodo": periodo},
            "cursos": cursos,
            "turmas": turmas,
            "anos": anos,
            "meses": meses,
...
def exportar_historico(request, aluno_cpf):
    ...
    try:
        ...
        response = HttpResponse(content_type="text/csv")
        response["Content-Disposition"] = (
            f'attachment; filename="historico_frequencia_{aluno.nome}.csv"'
        )
