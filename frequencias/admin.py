...
class CarenciaInline(admin.TabularInline):
    model = Carencia
    extra = 0
    readonly_fields = ['aluno', 'total_presencas', 'total_atividades', 'percentual_presenca', 'numero_carencias']
    fields = ['aluno', 'total_presencas', 'total_atividades', 'percentual_presenca', 'numero_carencias', 'liberado', 'observacoes']
    can_delete = False
...
class FrequenciaMensalAdmin(admin.ModelAdmin):
    ...
    def recalcular_carencias(self, request, queryset):
        ...
        self.message_user(request, f"Carências recalculadas para {queryset.count()} frequências.")
...
class CarenciaAdmin(admin.ModelAdmin):
    ...
    list_display = ('aluno', 'frequencia_mensal', 'percentual_presenca', 
                   'numero_carencias', 'liberado', 'status')
    ...
    date_hierarchy = 'created_at'
    readonly_fields = ['frequencia_mensal', 'aluno', 'total_presencas', 'total_atividades', 'percentual_presenca', 'numero_carencias']
    fieldsets = [
        ('Informações da Carência', {
            'fields': ['frequencia_mensal', 'aluno', 'total_presencas', 'total_atividades', 'percentual_presenca', 'numero_carencias']
        }),
        ('Status e Resolução', {
            'fields': ['liberado', 'status', 'observacoes', 'data_identificacao', 'data_acompanhamento', 'acompanhado_por', 'data_resolucao', 'resolvido_por', 'motivo_resolucao', 'observacoes_resolucao']
        }),
    ]
