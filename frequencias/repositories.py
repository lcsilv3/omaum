...
class FrequenciaMensalRepository:
    ...
    def buscar_por_id(frequencia_id):
        ...
        try:
            return FrequenciaMensal.objects.select_related("turma").get(
                id=frequencia_id
            )
    ...
    def buscar_por_turma(turma_id):
        ...
        return FrequenciaMensal.objects.filter(turma_id=turma_id).order_by(
            "-ano", "-mes"
        )
    ...
    def verificar_existencia(turma_id, mes, ano):
        ...
        return FrequenciaMensal.objects.filter(
            turma_id=turma_id, mes=mes, ano=ano
        ).exists()
    ...
    def obter_estatisticas_por_turma(turma_id):
        ...
        if total_frequencias == 0:
            return {"total_frequencias": 0, "percentual_medio": 0, "por_mes": []}
        ...
        for frequencia in frequencias:
            ...
            percentual_liberacao = (
                (liberados / total_alunos * 100) if total_alunos > 0 else 0
            )
        ...
        # Percentual médio
        percentuais = [
            item["percentual_liberacao"] for item in por_mes if item["total_alunos"] > 0
        ]
...
class CarenciaRepository:
    ...
    def listar_todas():
        ...
        return Carencia.objects.select_related(
            "frequencia_mensal", "frequencia_mensal__turma", "aluno"
        ).all()
    ...
    def buscar_por_frequencia(frequencia_id):
        ...
        return (
            Carencia.objects.filter(frequencia_mensal_id=frequencia_id)
    ...
    def buscar_por_aluno(aluno_cpf):
        ...
        return (
            Carencia.objects.filter(aluno__cpf=aluno_cpf)
            .select_related("frequencia_mensal", "frequencia_mensal__turma")
    ...
    def buscar_por_status(status):
        ...
        return (
            Carencia.objects.filter(status=status)
            .select_related("frequencia_mensal", "frequencia_mensal__turma", "aluno")
            .order_by("-created_at")
        )
    ...
    def buscar_nao_liberadas():
        ...
        return (
            Carencia.objects.filter(liberado=False)
            .select_related("frequencia_mensal", "frequencia_mensal__turma", "aluno")
            .order_by("-frequencia_mensal__ano", "-frequencia_mensal__mes")
        )
    ...
    def buscar_com_filtros(filtros):
        ...
        if filtros.get("turma_id"):
            carencias = carencias.filter(
                frequencia_mensal__turma_id=filtros["turma_id"]
            )
    ...
    def contar_por_status():
        ...
        return (
            Carencia.objects.values("status")
    ...
    def contar_por_turma():
        ...
        return (
            Carencia.objects.values(
                "frequencia_mensal__turma__nome", "frequencia_mensal__turma__id"
            )
            .annotate(
                total=Count("id"),
    ...
    def obter_alunos_problematicos(limite_carencias=3, periodo_meses=6):
        ...
        mes_limite = (
            hoje.month - periodo_meses
            if hoje.month > periodo_meses
            else 12 + (hoje.month - periodo_meses)
        )

        return (
            Carencia.objects.filter(
                frequencia_mensal__ano__gte=ano_limite,
                frequencia_mensal__mes__gte=mes_limite
                if ano_limite == hoje.year
                else 1,
                liberado=False,
            )
            ...
            .filter(total_carencias__gte=limite_carencias)
    ...
    def obter_estatisticas_gerais():
        ...
        por_periodo = (
            Carencia.objects.filter(
                ...
                | Q(
                    frequencia_mensal__ano=ano_inicio,
                    frequencia_mensal__mes__gte=mes_inicio,
                )
            )
            .values("frequencia_mensal__ano", "frequencia_mensal__mes")
            .annotate(
                total=Count("id"), nao_liberadas=Count("id", filter=Q(liberado=False))
            )
            .order_by("frequencia_mensal__ano", "frequencia_mensal__mes")
        )
        ...
        return {
            ...
            "medias": {
                "media_presenca": round(
                    float(stats_numericas["media_presenca"] or 0), 2
                ),
                "media_carencias": round(
                    float(stats_numericas["media_carencias"] or 0), 2
                ),
    ...
    def verificar_carencia_existente(frequencia_id, aluno_cpf):
        ...
        return Carencia.objects.filter(
            frequencia_mensal_id=frequencia_id, aluno__cpf=aluno_cpf
        ).exists()
    ...
    def obter_historico_aluno(aluno_cpf, limite=10):
        ...
        return (
            Carencia.objects.filter(aluno__cpf=aluno_cpf)
            .select_related("frequencia_mensal", "frequencia_mensal__turma")
            .order_by("-frequencia_mensal__ano", "-frequencia_mensal__mes")[:limite]
        )
