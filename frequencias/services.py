...
def get_frequencia_models():
    ...
    try:
        NotificacaoCarencia = getattr(
            import_module("frequencias.models"), "NotificacaoCarencia", None
        )
...
def listar_frequencias_mensais():
    ...
    return (
        modelos["FrequenciaMensal"]
        .objects.select_related("turma")
        .order_by("-ano", "-mes")
    )
...
def obter_carencias_por_frequencia(frequencia_id, apenas_pendentes=False):
    ...
    carencias = (
        modelos["Carencia"]
        .objects.filter(frequencia_mensal_id=frequencia_id)
...
def iniciar_acompanhamento_carencia(carencia_id, observacoes=""):
    ...
    """
    try:
        dados_atualizacao = {"status": "EM_ACOMPANHAMENTO", "observacoes": observacoes}
...
def obter_estatisticas_frequencia(frequencia_id):
    ...
    try:
        ...
        if total_alunos > 0:
            media_presenca = (
                carencias.aggregate(media=models.Avg("percentual_presenca"))["media"]
                or 0
            )
        else:
            media_presenca = 0
        ...
        return {
            ...
            "totais": {
                ...
                "percentual_liberados": (liberados / total_alunos * 100)
                if total_alunos > 0
                else 0,
            },
            "status_carencias": {
...
def gerar_relatorio_frequencias(filtros=None):
    ...
    try:
        ...
        if relatorio_dados:
            ...
            media_liberacao = (
                sum(item["totais"]["percentual_liberados"] for item in relatorio_dados)
                / total_frequencias
            )
            total_alunos_geral = sum(
                item["totais"]["total_alunos"] for item in relatorio_dados
            )
            total_liberados_geral = sum(
                item["totais"]["liberados"] for item in relatorio_dados
            )
        else:
            total_frequencias = 0
            media_liberacao = 0
        ...
        return {
            ...
            "resumo": {
                ...
                "percentual_geral": (total_liberados_geral / total_alunos_geral * 100)
...
def calcular_frequencia_aluno_periodo(aluno_cpf, data_inicio, data_fim):
    ...
    try:
        ...
        presencas = Presenca.objects.filter(
            aluno=aluno, data__gte=data_inicio, data__lte=data_fim
        )
        ...
        return {
            "aluno": {"cpf": aluno.cpf, "nome": aluno.nome},
