...
class FrequenciaViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {"error": "Frequência não encontrada"}, status=status.HTTP_404_NOT_FOUND
            )
        except Exception as e:
            return Response(
    ...
    def update(self, request, pk=None):
        ...
        try:
            ...
            if serializer.is_valid():
                frequencia_atualizada = service.update_frequencia(
...
def obter_dados_painel_frequencias(request):
    ...
    try:
        ...
        if not turma_id:
            return JsonResponse(
                {"success": False, "message": "Turma não especificada"}, status=400
            )
        ...
        for frequencia in frequencias_filtradas:
            ...
            alunos_carencia = carencias.filter(
                percentual_presenca__lt=frequencia.percentual_minimo
            ).count()
            ...
            frequencias_data.append(
                {
                    ...
                    "percentual_carencia": (alunos_carencia / total_alunos * 100)
                    if total_alunos > 0
                    else 0,
                }
            )
        ...
        logger.error(
            f"Erro ao obter dados do painel de frequências: {str(e)}", exc_info=True
        )
        return JsonResponse(
            {"success": False, "message": f"Erro ao obter dados do painel: {str(e)}"},
            status=400,
        )
...
def obter_turmas_por_curso(request, curso_id):
    ...
    try:
        ...
        turmas_data = []
        for turma in turmas:
            turmas_data.append({"id": turma.id, "nome": turma.nome})

        return JsonResponse({"success": True, "turmas": turmas_data})

    except Exception as e:
        logger.error(f"Erro ao obter turmas por curso: {str(e)}", exc_info=True)
        return JsonResponse(
            {"success": False, "message": f"Erro ao obter turmas: {str(e)}"}, status=400
        )
