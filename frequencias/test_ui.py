...
class FrequenciasUITest(StaticLiveServerTestCase):
    ...
    def test_fluxo_completo_usuario(self):
        ...
        header = None
        try:
            header = self.driver.find_element(By.XPATH, "//h1[contains(text(), 'Frequência')]")
        except Exception:
            try:
                header = self.driver.find_element(By.XPATH, "//*[self::h2 or self::h3][contains(text(), 'Frequência')]")
            except Exception:
        ...
        time.sleep(1)
        self.driver.find_element(By.NAME, 'nome').send_keys('Frequência Selenium')
        self.driver.find_element(By.NAME, 'descricao').send_keys('Frequência criada via Selenium')
