...
class MatriculaRepository:
    ...
    def buscar_por_aluno_e_turma(aluno_cpf, turma_id):
        ...
        return Matricula.objects.filter(aluno__cpf=aluno_cpf, turma_id=turma_id).first()
    ...
    def contar_por_status():
        ...
        return (
            Matricula.objects.values("status")
    ...
    def contar_por_turma():
        ...
        return (
            Matricula.objects.values("turma__nome", "turma__id")
            .annotate(total=Count("id"), ativas=Count("id", filter=Q(status="A")))
    ...
    def matriculas_canceladas():
        ...
        return Matricula.objects.filter(status="C").select_related(
            "aluno", "turma", "turma__curso"
        )
    ...
    def matriculas_finalizadas():
        ...
        return Matricula.objects.filter(status="F").select_related(
            "aluno", "turma", "turma__curso"
        )
    ...
    def matriculas_por_periodo(data_inicio, data_fim):
        ...
        return (
            Matricula.objects.filter(
                data_matricula__gte=data_inicio, data_matricula__lte=data_fim
            )
    ...
    def obter_estatisticas_gerais():
        ...
        matriculas_recentes = (
            Matricula.objects.filter(data_matricula__gte=seis_meses_atras)
            .values("data_matricula__year", "data_matricula__month")
            .annotate(total=Count("id"))
