...
class TurmaViewTest(TestCase):
    ...
    def test_atualizar_turma(self):
        response = self.client.post(
            reverse("turmas:turma_update", args=[self.turma.id]),
            {
    ...
    def test_deletar_turma(self):
        response = self.client.post(
            reverse("turmas:turma_delete", args=[self.turma.id])
        )
        self.assertEqual(
            response.status_code, 302
...
class CursoViewTest(TestCase):
    ...
    def test_criar_curso(self):
        response = self.client.post(
            reverse("turmas:curso_create"),
            {"nome": "Novo Curso", "descricao": "Descrição do novo curso"},
        )
        self.assertEqual(
            response.status_code, 302
        )  # Redirect after successful creation
    ...
    def test_atualizar_curso(self):
        response = self.client.post(
            reverse("turmas:curso_update", args=[self.curso.id]),
            {"nome": "Curso Atualizado", "descricao": "Descrição atualizada"},
        )
        self.assertEqual(
            response.status_code, 302
        )  # Redirect after successful update
    ...
    def test_deletar_curso(self):
        response = self.client.post(
            reverse("turmas:curso_delete", args=[self.curso.id])
        )
        self.assertEqual(
            response.status_code, 302
        )  # Redirect after successful deletion
