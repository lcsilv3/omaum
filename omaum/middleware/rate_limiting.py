...
class RateLimitMiddleware(MiddlewareMixin):
    ...
    def _create_rate_limit_response(self, limit_info: Dict[str, Any]) -> JsonResponse:
        ...
        response["X-RateLimit-Remaining"] = str(
            max(0, limit_info["limit"] - limit_info["current"])
        )
...
class APIThrottlingMixin:
    ...
    def check_throttling(
        self, request: HttpRequest, action: str = "default"
        ...
        if burst_count >= limits["burst_limit"]:
            return self._create_throttle_response(
                "burst", limits["burst_limit"], burst_count
            )
    ...
    def _create_throttle_response(
        self, limit_type: str, limit: int, current: int
    ) -> JsonResponse:
