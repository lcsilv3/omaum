...
def exemplo_uso():
    ...
    print(f"[INFO] Dados disponíveis:")
    ...
    if total_alunos > 0:
        ...
        try:
            ...
            print(f"   [OK] Sucesso! Percentual: {consolidado['percentuais']['presenca']}%")
            print(f"   [INFO] Total convocações: {consolidado['totais']['convocacoes']}")
    ...
    if total_turmas > 0:
        ...
        try:
            ...
            print(f"   [INFO] Percentual médio: {tabela['estatisticas_gerais']['percentual_medio']}%")
        except Exception as e:
            print(f"   [ERRO] Erro: {e}")
        print()
    ...
    if total_turmas > 0:
        ...
        try:
            ...
            print(f"   [OK] Sucesso! Alunos na turma: {estatisticas['totais']['alunos']}")
            print(f"   [INFO] Atividades: {estatisticas['totais']['atividades']}")
            print(f"   [INFO] Presença média: {estatisticas['percentuais']['presenca_media']}%")
        except Exception as e:
            print(f"   [ERRO] Erro: {e}")
        print()
    ...
    if total_configs > 0:
        ...
        print(f"   [INFO] Limites de carência:")
    ...
    if total_presencas > 0:
        ...
        try:
            if total_turmas > 0:
                ...
                resultado = CalculadoraEstatisticas.recalcular_todas_carencias()
                print(f"   [OK] Recálculo geral concluído!")
                print(f"   [INFO] Total processadas: {resultado['total_presencas']}")
        except Exception as e:
    ...
    print("   1. Importe: from presencas.services.calculadora_estatisticas import CalculadoraEstatisticas")
    print("   2. Use os métodos estáticos: CalculadoraEstatisticas.calcular_consolidado_aluno(aluno_id)")
