...
presencas_recentes = Presenca.objects.filter(data_registro__gte=ontem).order_by(
    "-data_registro"
)
...
try:
    ...
    presencas_turma = Presenca.objects.filter(turma=turma).order_by("-data_registro")[
        :5
    ]
    print(
        f'📊 Presenças da turma "{turma.nome}": {Presenca.objects.filter(turma=turma).count()} total'
    )
