...
def preencher_fotos():
    ...
    alunos_sem_foto = Aluno.objects.filter(
        Q(foto__isnull=True) | Q(foto=""), ativo=True
    )
    ...
    print(
        f"Encontrados {alunos_sem_foto.count()} alunos ativos sem foto. Iniciando o preenchimento..."
    )
