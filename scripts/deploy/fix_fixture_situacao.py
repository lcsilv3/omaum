    ...
    for item in data:
        if item.get("model") == "alunos.aluno":
            ...
            if "situacao" in fields and fields["situacao"] in SITUACAO_MAP:
                ...
                print(
                    f"  Aluno pk={item['pk']}: situacao '{old_value}' -> '{fields['situacao']}'"
                )
                total_fixed += 1

            # Corrigir campo situacao_iniciatica
            if (
                "situacao_iniciatica" in fields
                and fields["situacao_iniciatica"] in SITUACAO_MAP
            ):
                old_value = fields["situacao_iniciatica"]
                fields["situacao_iniciatica"] = SITUACAO_MAP[old_value]
                print(
                    f"  Aluno pk={item['pk']}: situacao_iniciatica '{old_value}' -> '{fields['situacao_iniciatica']}'"
                )
                total_fixed += 1
