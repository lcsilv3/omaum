    ...
    try:
        ...
        if turmas_disponiveis and random.random() > 0.1: # 90% de chance de matricular
            ...
            for turma in turmas_selecionadas:
                ...
                if not Matricula.objects.filter(aluno=aluno, turma__curso=turma.curso).exists():
        ...
        print(f'({i+1}/{QUANTIDADE_ALUNOS}) Aluno "{aluno.nome}" criado com sucesso com histórico!')
        alunos_criados += 1
