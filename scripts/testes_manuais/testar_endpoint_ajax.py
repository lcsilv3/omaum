...
def testar_endpoint_ajax():
    ...
    user, created = User.objects.get_or_create(
        username="teste_ajax", defaults={"email": "teste@teste.com"}
    )
    if created:
    ...
    print("📤 Enviando POST para o endpoint...")

    # Fazer requisição
    response = client.post(
        "/presencas/registrar-presenca/dias-atividades/ajax/", post_data
    )
    ...
    recentes = Presenca.objects.filter(
        data_registro__gte=agora - timedelta(minutes=5), registrado_por=user.username
    ...
    if recentes.exists():
        for p in recentes:
            print(
                f"   ✅ {p.aluno.nome} - {p.atividade.nome if p.atividade else 'N/A'} - {p.data} - {'Presente' if p.presente else 'Ausente'}"
            )
    else:
