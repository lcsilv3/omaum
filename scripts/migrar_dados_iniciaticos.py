    ...
    for aluno in Aluno.objects.all():
        try:
            with transaction.atomic():
                ...
                registros = (
                    RegistroHistorico.objects.filter(aluno=aluno)
                ...
                for registro in registros:
                    evento = {
                        ...
                        "tipo": registro.codigo.tipo_codigo.nome
                        if registro.codigo and registro.codigo.tipo_codigo
                        else "INDEFINIDO",
                        "descricao": registro.codigo.nome
                        if registro.codigo
                        else "Sem descrição",
                        "data": registro.data_os.isoformat()
                        if registro.data_os
                        else None,
                ...
                if historico_eventos:
                    ultimo_evento = max(
                        historico_eventos, key=lambda x: x["data"] or "1900-01-01"
                    )
                    ...
                    eventos_iniciacao = [
                        e for e in historico_eventos if "INICIAÇÃO" in e["tipo"].upper()
                    ]
                    if eventos_iniciacao:
                        primeiro_evento = min(
                            eventos_iniciacao, key=lambda x: x["data"] or "9999-12-31"
                        )
                        if primeiro_evento["data"]:
                            aluno.data_iniciacao = datetime.fromisoformat(
                                primeiro_evento["data"]
                            ).date()
        ...
        backup_data["codigos"].append(
            {
                ...
                "tipo_codigo_nome": codigo.tipo_codigo.nome
                if codigo.tipo_codigo
                else None,
            }
        )
            ...
            backup_data["registros_historico"].append(
                {
                    ...
                    "data_os": registro.data_os.isoformat()
                    if registro.data_os
                    else None,
