...
def test_form_data_processing():
    ...
    for atividade_index, atividade, dias in atividades_processadas:
        print(f"ATIVIDADE {atividade_index}: {atividade['nome']} (ID: {atividade['id']})")
        ...
        for aluno_cpf in alunos_ids:
            ...
            print(f"    Registros a criar:")
            for dia in dias:
                data_simulada = f"2024-01-{dia:02d}"
                print(f"      - Data: {data_simulada}, Presente: {presente}, Justificativa: '{justificativa}'")
            print()
