...
def teste_unique_together():
    ...
    try:
        ...
        print(f"   ✅ Primeira presença: {'criada' if created1 else 'atualizada'} - ID: {presenca1.id}")
        ...
        print(f"   ✅ Segunda presença: {'criada' if created2 else 'atualizada'} - ID: {presenca2.id}")
        
        # Verificar se são o mesmo objeto
        if presenca1.id == presenca2.id:
            print("   ✅ Sucesso! As presenças são o mesmo objeto (unique_together funcionando)")
        else:
        ...
        print(f"   📊 Total de presenças para este aluno/turma/data/atividade: {total_presencas}")
        
        if total_presencas == 1:
            print("   ✅ Perfeito! Apenas uma presença existe (unique_together funcionando)")
        else:
            print(f"   ❌ Problema! Existem {total_presencas} presenças (deveria ser 1)")
