    ...
    for pagamento in pagamentos:
        writer.writerow(
            [
                ...
                if hasattr(pagamento, "get_metodo_pagamento_display")
                else "-",
...
def generate_excel(pagamentos):
    ...
    for row, pagamento in enumerate(pagamentos, start=1):
        ...
        worksheet.write(
            row,
            5,
            pagamento.data_pagamento if pagamento.data_pagamento else "-",
            date_format if pagamento.data_pagamento else None,
        )
        worksheet.write(
            row,
            6,
