...
class IniciacaoViewAvancadoTest(TestCase):
    ...
    def test_paginacao(self):
        ...
        response = self.client.get(
            f"{reverse('iniciacoes:listar_iniciacoes')}?page=2"
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
    ...
    def test_filtro_aluno(self):
        url = (
            f"{reverse('iniciacoes:listar_iniciacoes')}?aluno={self.aluno1.id}"
        )
    ...
    def test_criar_iniciacao_duplicada(self):
        ...
        iniciacao_existente = Iniciacao.objects.filter(
            aluno=self.aluno1
        ).first()
        ...
        response = self.client.post(
            reverse("iniciacoes:criar_iniciacao"), form_data
        )
