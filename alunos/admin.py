...
class AlunoAdmin(admin.ModelAdmin):
    ...
    def historico_display(self, obj):
        ...
        if not obj.historico_iniciatico or not isinstance(
            obj.historico_iniciatico, list
        ):
        ...
        for evento in obj.obter_historico_ordenado():
            ...
            html += (
                f"<td style='border: 1px solid #ddd; padding: 8px;'>{descricao}</td>"
            )
            html += (
                f"<td style='border: 1px solid #ddd; padding: 8px;'>{observacoes}</td>"
            )
            html += "</tr>"
