...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "total",
            type=int,
            help="Indica o número de alunos a serem criados.",
            default=50,
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        if not turmas:
            self.stdout.write(
                self.style.ERROR(
                    "Nenhuma turma ativa encontrada. Por favor, crie e ative algumas turmas primeiro."
                )
            )
            return
        ...
        for i in range(total):
            ...
            try:
                ...
                if aluno:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f'Aluno "{nome}" criado e matriculado na turma "{turma_aleatoria.nome}" com sucesso.'
                        )
                    )
                    alunos_criados += 1
                else:
                    self.stdout.write(
                        self.style.ERROR(f"Falha ao criar o aluno {nome}.")
                    )

            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(
                        f"Ocorreu um erro inesperado ao processar o aluno {nome}: {e}"
                    )
                )

        self.stdout.write(
            self.style.SUCCESS(
                f"Processo concluído. {alunos_criados} de {total} alunos criados e matriculados."
            )
        )
