        ...
        try:
            with open(csv_file_path, mode='r', encoding='latin-1') as file:
                ...
                with transaction.atomic():
                    for row in reader:
                        try:
                            codigo_val = int(row['CODIGO'])
                            tipo_nome = row['TIPO'].strip()
                            descricao = row['DESCRICAO'].strip()

                            if not tipo_nome or not descricao:
                                self.stdout.write(self.style.WARNING(f'Linha ignorada por dados ausentes: {row}'))
                                continue

                            # Cria ou obtém o TipoCodigo
                            tipo, created = TipoCodigo.objects.get_or_create(nome=tipo_nome)
                            if created:
                                tipos_criados += 1
                                self.stdout.write(self.style.SUCCESS(f'Novo tipo criado: "{tipo_nome}"'))

                            # Cria o Codigo se ele não existir
                            ...
                            self.stdout.write(self.style.ERROR(f'Um erro inesperado ocorreu na linha {row}: {e}'))


            self.stdout.write(self.style.SUCCESS('\n--- Resumo da Importação ---'))
            self.stdout.write(self.style.SUCCESS(f'Tipos de Código criados: {tipos_criados}'))
            self.stdout.write(self.style.SUCCESS(f'Códigos criados: {codigos_criados}'))
            self.stdout.write(self.style.WARNING(f'Códigos ignorados (já existentes): {codigos_ignorados}'))
