        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    aluno = form.save()
                    messages.success(
                        request, f"Aluno '{aluno.nome}' criado com sucesso!"
                    )
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    aluno = form.save()
                    messages.success(
                        request, f"Aluno '{aluno.nome}' atualizado com sucesso!"
                    )
                    return redirect("alunos:detalhar_aluno_simple", aluno_id=aluno.cpf)
            except Exception as e:
...
def detalhar_aluno_simple(request, aluno_id):
    ...
    try:
        ...
        ultima_matricula = (
            Matricula.objects.filter(aluno=aluno, ativa=True)
...
def adicionar_evento_historico_ajax(request, aluno_id):
    ...
    try:
        ...
        if not all([tipo, descricao, data_evento]):
            return JsonResponse(
                {"success": False, "error": "Tipo, descrição e data são obrigatórios."},
                status=400,
            )
        ...
        aluno.adicionar_evento_historico(
            tipo=tipo, descricao=descricao, data=data_obj, observacoes=observacoes
        )
