...
def _converter_eventos(eventos: Iterable[Mapping[str, Any]] | None) -> list[dict[str, Any]]:
...
def criar_aluno(
	...
	*,
	...
	eventos_iniciais = eventos_historico or dados_limpos.pop("historico_iniciatico", None)
	...
	try:
		with transaction.atomic():
			...
			if eventos_iniciais:
				...
				aluno.save(update_fields=["historico_iniciatico", "historico_checksum", "updated_at"])
...
class InstrutorService:
	...
	@staticmethod
	def remover_de_turmas(aluno: Aluno | None, nova_situacao: str | None = None) -> dict:
	...
	def verificar_elegibilidade_completa(aluno: Aluno | None) -> dict[str, Any]:
		...
		motivo = (
			"Aluno elegível para atuar como instrutor."
			if elegivel
			else pendencias[0]
		)
