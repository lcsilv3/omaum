...
def verificar_elegibilidade_endpoint(request, cpf):
    ...
    try:
        ...
        if hasattr(aluno, 'pode_ser_instrutor'):
            ...
            logger.warning(f"Método 'pode_ser_instrutor' não encontrado para o aluno {cpf}")
            return JsonResponse({"elegivel": True})
        
        return JsonResponse({"elegivel": True})
    except Exception as e:
        logger.error(f"Erro ao verificar elegibilidade do aluno {cpf}: {str(e)}", exc_info=True)
        return JsonResponse(
