...
def relatorio_aniversariantes(request):
    ...
    if export in ["csv", "xls", "pdf"]:
        now = datetime.now()
        report_header_org = "OM-AUM (Ordem Mística de Aspiração Universal ao Mestrado)"
        report_header_datetime = f"Emitido em: {now.strftime('%d/%m/%Y às %H:%M')}"
        ...
        if export == "csv":
            ...
            writer.writerow([report_header_org])
            writer.writerow([report_header_datetime])
            writer.writerow([])
            ...
            ws.write(0, 0, report_header_org)
            ws.write(1, 0, report_header_datetime)
            ...
            for row_idx, aluno in enumerate(alunos_qs, start=4):
                ...
                ws.write(row_idx, 2, smart_str(aluno.data_nascimento))
                ws.write(row_idx, 3, smart_str(aluno.email))
                ws.write(row_idx, 4, smart_str(aluno.celular_primeiro_contato or "-"))
            output = BytesIO()
            wb.save(output)
            output.seek(0)
...
def painel_kpis_api(request):
    ...
    media_idade = Aluno.objects.filter(situacao="a").aggregate(m=Avg("data_nascimento"))['m']
...
def painel_graficos_api(request):
    ...
    for m in meses:
        ...
        valores_mes.append(Aluno.objects.filter(created_at__date__gte=m, created_at__date__lt=prox).count())
    return JsonResponse({
...
def painel_tabela_api(request):
    ...
    html = render_to_string("alunos/_tabela_alunos_parcial.html", {"alunos": alunos_page}, request=request)
    paginacao_html = ""
    if alunos_page.paginator.num_pages > 1:
        ...
        for p in alunos_page.paginator.page_range:
            ...
            paginacao_html += (
                f'<li class="page-item{active}"><a href="#" class="page-link" data-page="{p}">{p}</a></li>'
            )
        paginacao_html += "</ul>"
...
def relatorio_ficha_cadastral(request):
    ...
    if export in ["csv", "xls"]:
        if export == "csv":
            ...
            writer = csv.writer(response)
            writer.writerow(["Nome", "CPF", "Data Nascimento", "Email", "Situação", "Endereço", "Turma", "Curso"])
            for aluno in alunos_qs:
                endereco = (
                    f"{aluno.rua or ''}, {aluno.numero_imovel or ''}, {aluno.bairro_ref or ''}, {aluno.cidade_ref or ''}, {aluno.cep or ''}"
                )
                turma_nome = getattr(getattr(aluno, "matricula_set", None).first(), "turma", None)
            ...
            headers = ["Nome", "CPF", "Data Nascimento", "Email", "Situação", "Endereço", "Turma", "Curso"]
            for col, h in enumerate(headers):
                ws.write(0, col, h)
            for row, aluno in enumerate(alunos_qs, start=1):
                endereco = (
    ...
    return render(request, "alunos/relatorio_ficha_cadastral.html", {
        "alunos": alunos_qs,
        "turmas": turmas,
        "cursos": cursos,
        "situacoes": situacoes,
...
def relatorio_dados_iniciaticos(request):
    ...
    if export in ["csv", "xls"]:
        if export == "csv":
            ...
            writer = csv.writer(response)
            writer.writerow(["Nome", "CPF", "Grau Atual", "Situação", "Tempo de Casa (anos)", "Data Ingresso"])
            for aluno in alunos_qs:
                tempo = (date.today() - aluno.created_at.date()).days // 365 if aluno.created_at else "-"
                writer.writerow([
                    smart_str(aluno.nome),
                    smart_str(aluno.cpf),
            ...
            headers = ["Nome", "CPF", "Grau Atual", "Situação", "Tempo de Casa (anos)", "Data Ingresso"]
            for col, h in enumerate(headers):
                ws.write(0, col, h)
            for row, aluno in enumerate(alunos_qs, start=1):
                tempo = (date.today() - aluno.created_at.date()).days // 365 if aluno.created_at else "-"
                ...
                ws.write(row, 5, smart_str(aluno.created_at.date() if aluno.created_at else "-"))
            output = BytesIO()
            wb.save(output)
            output.seek(0)
            response = HttpResponse(output.getvalue(), content_type="application/vnd.ms-excel")
            response["Content-Disposition"] = 'attachment; filename="dados_iniciaticos.xls"'
            return response
    graus = Aluno.objects.values_list("grau_atual", flat=True).distinct().order_by("grau_atual")
    situacoes = Aluno.SITUACAO_CHOICES
    return render(request, "alunos/relatorio_dados_iniciaticos.html", {
        "alunos": alunos_qs,
        "graus": graus,
        "situacoes": situacoes,
...
def relatorio_historico_aluno(request):
    ...
    if export in ["csv", "xls", "pdf"]:
        if export == "csv":
            ...
            writer = csv.writer(response)
            writer.writerow(["Aluno", "CPF", "Evento", "Data", "Ordem de Serviço", "Observações"])
            ...
            headers = ["Aluno", "CPF", "Evento", "Data", "Ordem de Serviço", "Observações"]
            for col, h in enumerate(headers):
                ws.write(0, col, h)
            ...
            output.seek(0)
            response = HttpResponse(output.getvalue(), content_type="application/vnd.ms-excel")
            response["Content-Disposition"] = 'attachment; filename="historico_aluno.xls"'
            return response
        elif export == "pdf":
            from weasyprint import HTML
            ...
            html_string = template.render({
                ...
                "filtros": {"aluno": aluno_id, "tipo_evento": tipo_evento, "data_ini": data_ini, "data_fim": data_fim},
                "is_export_pdf": True,
            })
            pdf_file = HTML(string=html_string, base_url=request.build_absolute_uri()).write_pdf()
            response = HttpResponse(pdf_file, content_type="application/pdf")
            response["Content-Disposition"] = 'attachment; filename="historico_aluno.pdf"'
            return response
    alunos = Aluno.objects.all().order_by("nome")
    tipos_evento = Codigo.objects.all().order_by("nome")
    return render(request, "alunos/relatorio_historico_aluno.html", {
        "historico": historico_qs,
        "alunos": alunos,
...
def relatorio_auditoria_dados(request):
    ...
    if export in ["csv", "xls"]:
        if export == "csv":
            ...
            output.seek(0)
            response = HttpResponse(output.getvalue(), content_type="application/vnd.ms-excel")
            response["Content-Disposition"] = 'attachment; filename="auditoria_dados.xls"'
            return response
    return render(request, "alunos/relatorio_auditoria_dados.html", {
        "alunos": alunos_qs,
...
def relatorio_demografico(request):
    ...
    if faixa:
        ...
        if faixa == "<18":
            ...
            alunos_qs = alunos_qs.filter(data_nascimento__lte=data_fim, data_nascimento__gt=data_ini)
    alunos_qs = alunos_qs.order_by("nome")
    if export in ["csv", "xls"]:
        if export == "csv":
            response = HttpResponse(content_type="text/csv")
            ...
            for row, aluno in enumerate(alunos_qs, start=1):
                ...
                ws.write(row, 4, smart_str(aluno.cidade_ref.nome if aluno.cidade_ref else "-"))
            output = BytesIO()
            wb.save(output)
            output.seek(0)
            response = HttpResponse(output.getvalue(), content_type="application/vnd.ms-excel")
            ...
            return response
    sexo_data = list(alunos_qs.values("sexo").annotate(qtd=Count("id")).order_by("sexo"))
    hoje = date.today()
    faixa_counts = {k: 0 for k, _ in faixas}
    for aluno in alunos_qs:
        idade = (hoje - aluno.data_nascimento).days // 365 if aluno.data_nascimento else 0
        if idade < 18:
            faixa_counts["<18"] += 1
        elif idade <= 25:
    ...
    return render(request, "alunos/relatorio_demografico.html", {
        "alunos": alunos_qs,
        "faixas": faixas,
        "cidades": cidades,
