...
def editar_presencas_lote(request):
    ...
    if request.method == 'POST':
        ...
        if presencas_existentes == 0:
            messages.warning(request, 'Não há presenças registradas para o período selecionado.')
...
def editar_lote_dados_basicos(request):
    ...
    if not all([turma_id, ano, mes]):
        messages.error(request, 'Dados de seleção não encontrados. Reinicie o processo.')
        return redirect('presencas:editar_presencas_lote')
    ...
    if request.method == 'POST':
        ...
        for presenca in presencas_periodo:
            pode_editar, motivo = PresencaPermissionEngine.pode_alterar_presenca(presenca, request.user)
            if not pode_editar:
                messages.error(request, f'Sem permissão para editar presença de {presenca.aluno.nome}: {motivo}')
                permissoes_ok = False
                break
        ...
        messages.success(request, 'Dados básicos validados. Prosseguindo para edição...')
...
def editar_lote_totais_atividades(request):
    ...
    for item in atividades_com_presenca:
        ...
        total_dias = presencas_periodo.filter(atividade_id=atividade_id).values('data').distinct().count()
        totais_atuais[f'qtd_ativ_{atividade_id}'] = total_dias
    
    # Obter atividades do período
    atividades = Atividade.objects.filter(id__in=[item['atividade'] for item in atividades_com_presenca])
    ...
    if request.method == 'POST':
        ...
        for key, value in request.POST.items():
            if key.startswith('qtd_ativ_') and value.isdigit():
                ...
                if novo_valor < valor_atual:
                    ...
                    atividade_nome = next((a.nome for a in atividades if str(a.id) == atividade_id), 'Desconhecida')
        ...
        if novos_totais:
            alteracoes = sum(1 for k, v in novos_totais.items() if v != totais_atuais.get(k, 0))
            if alteracoes > 0:
                messages.success(
                    request,
                    (
                ...
                messages.success(request, '✅ Configuração confirmada. Prosseguindo para seleção de dias...')
        else:
            messages.success(request, '✅ Configuração confirmada. Prosseguindo para seleção de dias...')
...
def editar_lote_dias_atividades(request):
    ...
    # Carregar dados das presenças existentes
    primeiro_dia = date(int(ano), int(mes), 1)
    ultimo_dia = date(int(ano), int(mes), monthrange(int(ano), int(mes))[1])
    ...
    # Organizar dados para o JavaScript
    presencas_por_atividade = {}
    dias_por_atividade = {}
    ...
    convs = ConvocacaoPresenca.objects.filter(
        turma=turma,
        data__range=[primeiro_dia, ultimo_dia]
    ).select_related('aluno', 'atividade')
    convoc_map = {}
    for c in convs:
        key = (str(c.atividade.id), c.data.day, c.aluno.cpf)
        convoc_map[key] = c.convocado

    for presenca in presencas_existentes:
        atividade_id = str(presenca.atividade.id)
        dia = presenca.data.day
        aluno_cpf = presenca.aluno.cpf
        
        if atividade_id not in presencas_por_atividade:
            presencas_por_atividade[atividade_id] = {}
            dias_por_atividade[atividade_id] = set()
        
        if dia not in presencas_por_atividade[atividade_id]:
            presencas_por_atividade[atividade_id][dia] = {}
        
        # Convocado default: True se atividade é de convocação, senão None/False
        convocado_val = convoc_map.get((atividade_id, dia, aluno_cpf))
        presencas_por_atividade[atividade_id][dia][aluno_cpf] = {
            'presente': presenca.presente,
            'justificativa': presenca.justificativa or '',
            'presenca_id': presenca.id,  # Para identificar na edição
            'convocado': convocado_val,
        }
        
        dias_por_atividade[atividade_id].add(dia)
    
    # Converter sets para listas e calcular estatísticas
    for atividade_id in dias_por_atividade:
        dias_por_atividade[atividade_id] = sorted(list(dias_por_atividade[atividade_id]))
    ...
    for atividade in atividades:
        ...
        total_dias_esperados = totais_atividades.get(novo_total_key, total_dias_existentes)
        
        # Adicionar atributos dinâmicos
        atividade.total_dias = total_dias_esperados  # Total esperado (pode ser maior que existente)
        atividade.total_dias_existentes = total_dias_existentes  # Dias realmente existentes
        atividade.total_presencas = total_presencas
        atividade.dias_adicionais_necessarios = max(0, total_dias_esperados - total_dias_existentes)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Limpar sessão
            session_keys = ['edicao_lote_turma_id', 'edicao_lote_ano', 'edicao_lote_mes', 'edicao_lote_totais_atividades']
                ...
                for dia, lista_cpfs in dias_exclusoes.items():
                    ...
                    for cpf_aluno in lista_cpfs:
                        ...
                        try:
                            ...
                            pode_excluir, motivo = PresencaPermissionEngine.pode_alterar_presenca(presenca_existente, request.user)
                            if not pode_excluir:
                                continue
                ...
                for dia, alunos_dados in dias_dados.items():
                    ...
                    for cpf_aluno, dados_presenca in alunos_dados.items():
                        ...
                        try:
                            ...
                            pode_editar, motivo = PresencaPermissionEngine.pode_alterar_presenca(presenca_existente, request.user)
                        except Presenca.DoesNotExist:
                            pass
                        ...
                        if presenca_existente:
                            # UPDATE
                            presenca_existente.presente = dados_presenca.get('presente', True)
                            presenca_existente.justificativa = dados_presenca.get('justificativa', '')
                        ...
                        if 'convocado' in dados_presenca:
                            try:
                                ConvocacaoPresenca.objects.update_or_create(
                                    ...
                                    data=data_presenca,
                                    defaults={
                                        'convocado': bool(dados_presenca.get('convocado')),
                                        'registrado_por': request.user.username,
                                    }
                                )
                            except Exception as e:
                                logger.warning(f"Falha ao atualizar convocação para {aluno.cpf} em {data_presenca}: {e}")
