...
class ProcessarExportacaoView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if formato == "csv":
                ...
                return JsonResponse(
                    {"erro": f"Formato não suportado: {formato}"}, status=400
                )
    ...
    def _obter_estatisticas_executivas(self, filtros: Dict[str, Any]) -> Dict[str, Any]:
        ...
        total_conv = (agregados["total_presencas"] or 0) + (
            agregados["total_faltas"] or 0
        )
        percentual_geral = (
    ...
    def _gerar_csv(
        self, template: str, dados: Dict[str, Any], config: Dict[str, Any]
    ) -> HttpResponse:
    ...
    def _gerar_excel_basico(
        ...
        response["Content-Disposition"] = (
            'attachment; filename="relatorio_presencas.xlsx"'
        )

        return response

    def _escrever_excel_consolidado_geral(
        self, ws, dados: Dict[str, Any], start_row: int
    ) -> int:
        ...
        for presenca in dados["presencas_detalhadas"]:
            ...
            ws.cell(
                row=start_row,
                column=4,
                value=presenca.atividade.data.strftime("%d/%m/%Y"),
            )
            ...
            ws.cell(
                row=start_row,
                column=8,
                value=f"{float(presenca.percentual_presenca):.2f}%",
            )
            start_row += 1

        return start_row

    def _escrever_excel_por_turma(
        self, ws, dados: Dict[str, Any], start_row: int
    ) -> int:
    ...
    def _escrever_excel_estatisticas(
        self, ws, dados: Dict[str, Any], start_row: int
    ) -> int:
...
class GerenciarAgendamentosView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["mensagem"] = (
            "Funcionalidade de agendamento será implementada em versão futura."
        )
