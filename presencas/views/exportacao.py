...
from ..models import PresencaDetalhada, Turma
...
class ProcessarExportacaoView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not formato or not template:
                return JsonResponse(
                    {"erro": "Formato e template são obrigatórios"}, status=400
                )
    ...
    def _extrair_configuracoes(self, request) -> Dict[str, Any]:
        ...
        return {
            ...
            "formatacao_profissional": request.POST.get("formatacao_profissional")
            == "on",
    ...
    def _processar_exportacao(
        self, formato: str, template: str, config: Dict[str, Any]
    ):
    ...
    def _obter_dados_por_turma(self, filtros: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for presenca in query:
            ...
            dados_por_turma[turma_id]["estatisticas"]["total_presencas"] += (
                presenca.presencas
            )
    ...
    def _obter_dados_por_curso(self, filtros: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for turma_data in dados_turma["dados_por_turma"].values():
            curso_nome = (
                turma_data["turma"].curso.nome
                if turma_data["turma"].curso
                else "Sem Curso"
            )
    ...
    def _obter_estatisticas_executivas(self, filtros: Dict[str, Any]) -> Dict[str, Any]:
        ...
        estatisticas = {
            "resumo_geral": {
                ...
                "total_presencas": query.aggregate(Sum("presencas"))["presencas__sum"]
                or 0,
            ...
            "atividades_mais_participativas": self._obter_atividades_participativas(
                query
            ),
        }
        ...
        total_conv = (
            estatisticas["resumo_geral"]["total_presencas"]
        ...
        for dados_aluno in dados_por_aluno.values():
            if dados_aluno["total_convocacoes"] > 0:
                dados_aluno["percentual"] = (
                    dados_aluno["total_presencas"]
                    / dados_aluno["total_convocacoes"]
                    * 100
                )
    ...
    def _obter_dados_comparativo(self, filtros: Dict[str, Any]) -> Dict[str, Any]:
        ...
        agora = timezone.now()
        periodos = [
            {"nome": "Mês Atual", "inicio": agora.replace(day=1), "fim": agora},
            {
                "nome": "Mês Anterior",
    ...
    def _obter_top_alunos_presenca(
        self, query, limite: int = 10
    ) -> List[Dict[str, Any]]:
        ...
        # Ordenar e limitar
        top_alunos = sorted(
            alunos_presenca.values(), key=lambda x: x["percentual"], reverse=True
        )[:limite]
    ...
    def _obter_alunos_carencia(self, query, limite: int = 10) -> List[Dict[str, Any]]:
        ...
        alunos_carencia = [aluno for aluno in top_alunos if aluno["percentual"] < 75]
...
class ExcelAvancadoExporter:
    ...
    def _configurar_estilos(self):
        ...
        self.estilos["header_principal"].font = Font(bold=True, color="FFFFFF", size=12)
    ...
    def gerar_excel_consolidado_geral(
    ...
    def _criar_aba_consolidado_principal(
        self, ws, dados: Dict[str, Any], config: Dict[str, Any]
    ):
    ...
    def _criar_tabela_estatisticas_resumo(
        self, ws, start_row: int, stats: Dict[str, Any]
    ):
    ...
    def _criar_tabela_dados_consolidados(
        self, ws, start_row: int, dados: Dict[str, Any]
    ):
        ...
        dados_por_aluno = self._agrupar_dados_por_aluno(
            presencas_detalhadas, atividades
        )
        ...
        for aluno_dados in dados_por_aluno.values():
            ...
            # Dados básicos
            ws.cell(
                row=row, column=col, value=aluno_dados["aluno"].nome
            ).style = "dados"
            col += 1
            ws.cell(
                row=row, column=col, value=aluno_dados["turma"].nome
            ).style = "dados"
            col += 1
            ...
            for atividade in atividades:
                ...
                if presenca:
                    ws.cell(
                        row=row, column=col, value=presenca.convocacoes
                    ).style = "numerico"
                    ws.cell(
                        row=row, column=col + 1, value=presenca.presencas
                    ).style = "numerico"
                    ws.cell(
                        row=row, column=col + 2, value=presenca.faltas
                    ).style = "numerico"
            ...
            ws.cell(
                row=row, column=col + 1, value=totais["presencas"]
            ).style = "numerico"
    ...
    def _agrupar_dados_por_aluno(
        self, presencas_detalhadas, atividades
    ) -> Dict[int, Dict]:
    ...
    def _criar_aba_estatisticas(self, ws, estatisticas: Dict[str, Any]):
        ...
        # Resumo geral
        self._criar_secao_estatisticas(
            ws,
            row,
            "Resumo Geral",
            [
    ...
    def _criar_secao_estatisticas(
        self, ws, start_row: int, titulo: str, dados: List[List]
    ):
    ...
    def _criar_aba_dados_detalhados(self, ws, presencas_detalhadas):
        ...
        for presenca in presencas_detalhadas:
            ...
            perc_cell = ws.cell(
                row=row, column=8, value=float(presenca.percentual_presenca) / 100
            )
            perc_cell.style = "percentual"
            self._aplicar_formatacao_condicional_percentual(perc_cell)
