...
class PainelEstatisticasView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            context['dados_graficos'] = {
                'distribuicao_presencas': self._dados_grafico_distribuicao_presencas(filtros),
                ...
                'performance_atividades': self._dados_grafico_performance_atividades(filtros),
            }
    ...
    def _calcular_metricas_principais(self, filtros: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if total_convocacoes > 0:
                percentual_presenca = round((total_presencas / total_convocacoes) * 100, 2)
            ...
            return {
                'total_alunos': 0, 'total_turmas': 0, 'total_atividades': 0,
                'total_convocacoes': 0, 'total_presencas': 0, 'total_faltas': 0,
                'total_carencias': 0, 'percentual_presenca': 0.0,
                'percentual_faltas': 0.0, 'media_carencias': 0.0
            }
    
    def _dados_grafico_distribuicao_presencas(self, filtros: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _dados_grafico_evolucao_temporal(self, filtros: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for mes_ano in sorted(dados_mensais.keys()):
                ...
                if dados['convocacoes'] > 0:
                    perc_presenca = round((dados['presencas'] / dados['convocacoes']) * 100, 2)
                    perc_faltas = round((dados['faltas'] / dados['convocacoes']) * 100, 2)
                else:
                    perc_presenca = 0.0
                    perc_faltas = 0.0
            ...
            logger.error(f"Erro ao gerar dados do gráfico de evolução temporal: {str(e)}")
            return {
                'tipo': 'line',
                'labels': [],
                'datasets': []
    ...
    def _dados_grafico_carencias_turma(self, filtros: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if filtros.get('turma_id'):
                ...
                top_carencias = sorted(carencias_por_aluno.items(), key=lambda x: x[1], reverse=True)[:10]
                
                labels = [item[0][:20] + '...' if len(item[0]) > 20 else item[0] for item in top_carencias]
                ...
                top_carencias = sorted(carencias_por_turma.items(), key=lambda x: x[1], reverse=True)
    ...
    def _dados_grafico_performance_atividades(self, filtros: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for presenca in presencas_qs:
                ...
                performance_atividades[atividade_nome]['convocacoes'] += presenca.convocacoes
                performance_atividades[atividade_nome]['presencas'] += presenca.presencas
            ...
            for atividade, dados in performance_atividades.items():
                if dados['convocacoes'] > 0:
                    percentual = round((dados['presencas'] / dados['convocacoes']) * 100, 2)
    ...
    def _obter_filtros_disponiveis(self) -> Dict[str, List]:
        ...
        try:
            ...
            atividades = list(Atividade.objects.all().values('id', 'nome').order_by('nome'))
...
class PainelDadosAjaxView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not tipo_dado:
                return JsonResponse({'erro': 'Tipo de dado não especificado'}, status=400)
...
class ExportarRelatorioView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            if formato == 'json':
                ...
                response['Content-Disposition'] = 'attachment; filename="relatorio_painel.json"'
                return response
    ...
    def _exportar_csv(self, dados_relatorio: Dict[str, Any]) -> HttpResponse:
        ...
        # Cabeçalho
        writer.writerow(['Nome', 'CPF', 'Turma', 'Convocações', 'Presenças', 
                        'Faltas', 'Percentual', 'Carências', 'Status'])
    ...
    def _exportar_pdf(self, dados_relatorio: Dict[str, Any]) -> HttpResponse:
        ...
        html_content = render_to_string('presencas/relatorio_painel_pdf.html', {
