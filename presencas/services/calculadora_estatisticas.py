...
class CalculadoraEstatisticas:
        ...
        try:
            ...
            if total_convocacoes > 0:
                percentual_presenca = round(
                    (Decimal(total_presencas) / Decimal(total_convocacoes)) * Decimal('100'), 2
                )
            ...
            consolidado = {
                ...
                'percentuais': {
                    ...
                    'faltas': float(
                        round((Decimal(total_faltas) / Decimal(total_convocacoes)) * Decimal('100'), 2)
                        if total_convocacoes > 0 else Decimal('0.00')
                    )
                },
                'status': status,
    ...
    def gerar_tabela_consolidada(
        ...
        try:
            ...
            for presenca in presencas:
                ...
                dados_por_aluno[aluno_id]['totais']['voluntario_extra'] += presenca.voluntario_extra
                dados_por_aluno[aluno_id]['totais']['voluntario_simples'] += presenca.voluntario_simples
            ...
            for aluno_id, dados in dados_por_aluno.items():
                ...
                if totais['convocacoes'] > 0:
                    percentual_geral = round(
                        (Decimal(totais['presencas']) / Decimal(totais['convocacoes'])) * Decimal('100'), 2
                    )
                ...
                for atividade_key, atividade_data in dados['atividades'].items():
                    if atividade_data['convocacoes'] > 0:
                        atividade_data['percentual'] = round(
                            (Decimal(atividade_data['presencas']) / Decimal(atividade_data['convocacoes'])) * Decimal('100'), 2
                        )
                ...
                linha = {
                    ...
                    'total_voluntarios': totais['voluntario_extra'] + totais['voluntario_simples'],
                    'status': status,
            ...
            # Ordenar
            linhas_tabela = CalculadoraEstatisticas._ordenar_tabela(linhas_tabela, ordenar_por)
            
            # Estatísticas gerais
            estatisticas_gerais = CalculadoraEstatisticas._calcular_estatisticas_gerais(linhas_tabela)
        ...
        try:
            ...
            if agregacoes['total_convocacoes'] and agregacoes['total_convocacoes'] > 0:
                percentual_medio = round(
                    (Decimal(agregacoes['total_presencas']) / Decimal(agregacoes['total_convocacoes'])) * Decimal('100'), 2
                )
            ...
            estatisticas_alunos = CalculadoraEstatisticas._calcular_por_aluno(
                presencas
            )
            
            # Distribuição de carências
            distribuicao_carencias = CalculadoraEstatisticas._calcular_distribuicao_carencias(
                presencas
            )
            ...
            if presencas.exists():
                ...
                turma_info = {
                    ...
                    'perc_carencia': getattr(primeira_presenca.turma, 'perc_carencia', None),
                }
            ...
            estatisticas = {
                ...
                'totais': {
                    ...
                    'voluntario_extra': agregacoes['total_voluntario_extra'] or 0,
                    'voluntario_simples': agregacoes['total_voluntario_simples'] or 0,
                    'total_voluntarios': (agregacoes['total_voluntario_extra'] or 0) + (agregacoes['total_voluntario_simples'] or 0),
                ...
                'percentuais': {
                    ...
                    'faltas_media': float(
                        round((Decimal(agregacoes['total_faltas'] or 0) / Decimal(agregacoes['total_convocacoes'] or 1)) * Decimal('100'), 2)
                    )
                },
    ...
    def calcular_carencias(
        ...
        try:
            ...
            if not forcar_recalculo and hasattr(presenca, 'carencias') and presenca.carencias is not None:
                logger.info(f"Carência já calculada para presença {presenca_detalhada_id}")
                return {
            ...
            if configuracao:
                ...
                if presenca.convocacoes > 0:
                    ...
                    carencias_calculadas = max(0, presencas_necessarias - presenca.presencas)
                else:
                ...
                if hasattr(presenca.turma, 'perc_carencia') and presenca.turma.perc_carencia:
                    percentual_atual = presenca.calcular_percentual()
                    percentual_minimo = presenca.turma.perc_carencia
                    
                    if percentual_atual < percentual_minimo:
                        presencas_necessarias = (percentual_minimo * presenca.convocacoes) / 100
                        carencias_calculadas = int(presencas_necessarias - presenca.presencas)
            ...
            logger.error(f"Erro ao calcular carências para presença {presenca_detalhada_id}: {str(e)}")
    ...
    def _criar_consolidado_vazio(aluno_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'totais': {
                'convocacoes': 0, 'presencas': 0, 'faltas': 0,
                'voluntario_extra': 0, 'voluntario_simples': 0,
                'total_voluntarios': 0, 'carencias': 0, 'registros': 0
            },
    ...
    def _criar_estatisticas_turma_vazia(turma_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'totais': {
                'convocacoes': 0, 'presencas': 0, 'faltas': 0,
                'voluntario_extra': 0, 'voluntario_simples': 0,
                'total_voluntarios': 0, 'carencias': 0,
                'alunos': 0, 'atividades': 0
            },
            'percentuais': {'presenca_media': 0.0, 'faltas_media': 0.0},
            'por_atividade': [], 'por_aluno': [],
    ...
    def _calcular_por_atividade(presencas_queryset) -> List[Dict[str, Any]]:
        ...
        for atividade_data in atividades.values():
            ...
            atividade_data['total_voluntarios'] = atividade_data['voluntario_extra'] + atividade_data['voluntario_simples']
    ...
    def _calcular_por_aluno(presencas_queryset) -> List[Dict[str, Any]]:
        ...
        for aluno_data in alunos.values():
            ...
            if aluno_data['convocacoes'] > 0:
                percentual = round(
                    (Decimal(aluno_data['presencas']) / Decimal(aluno_data['convocacoes'])) * Decimal('100'), 2
                )
            
            aluno_data['percentual'] = float(percentual)
            aluno_data['total_voluntarios'] = aluno_data['voluntario_extra'] + aluno_data['voluntario_simples']
