...
class PerformanceTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username="perf_user", password="test123")

        # Configurar dados base
        self.turma = Turma.objects.create(
            codigo_turma="PERF001", nome="Turma Performance Test"
        )

        self.atividade = Atividade.objects.create(
            nome="Atividade Performance", tipo="AULA", ativa=True
        )
...
class LoadTestCase(PerformanceTestCase):
    ...
    def test_turma_100_alunos(self):
        ...
        self.assertLess(
            tempo_execucao, 3.0, f"Carregamento muito lento: {tempo_execucao}s"
        )
    ...
    def test_historico_1_ano_completo(self):
        ...
        self.assertLess(
            tempo_execucao, 5.0, f"Relatório anual muito lento: {tempo_execucao}s"
        )
    ...
    def test_exportacao_dados_grandes(self):
        ...
        self.assertLess(
            tempo_execucao, 10.0, f"Exportação muito lenta: {tempo_execucao}s"
        )
...
class ResponseTimeTest(PerformanceTestCase):
    ...
    def test_tempo_resposta_pagina_inicial(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertLess(
            tempo_resposta, 1.0, f"Página inicial muito lenta: {tempo_resposta}s"
        )

    def test_tempo_resposta_api_ajax(self):
        """Testa tempo de resposta das APIs AJAX."""
        self.criar_alunos_em_lote(15)
        ...
        self.assertEqual(response.status_code, 200)
        self.assertLess(tempo_resposta, 0.5, f"API AJAX muito lenta: {tempo_resposta}s")
    ...
    def test_tempo_resposta_salvamento_presenca(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertLess(
            tempo_resposta, 0.3, f"Salvamento muito lento: {tempo_resposta}s"
        )
...
class BenchmarkTest(PerformanceTestCase):
    ...
    def test_benchmark_criacao_presencas(self):
        ...
        total_criadas = PresencaAcademica.objects.filter(
            turma=self.turma, data=date.today()
        ).count()
    ...
    def test_benchmark_consulta_historico(self):
        ...
        resultado = (
            PresencaAcademica.objects.filter(turma=self.turma, data__gte=data_inicio)
            .values("aluno__nome")
            .annotate(
        ...
        self.assertLess(
            tempo_consulta, 0.5, f"Consulta de histórico muito lenta: {tempo_consulta}s"
        )
    ...
    def test_benchmark_calculo_estatisticas(self):
        ...
        estatisticas = {
            ...
            "alunos_frequentes": presencas_queryset.values("aluno")
            .annotate(
                taxa_presenca=Count("id", filter=Q(presente=True)) * 100.0 / Count("id")
            )
            .filter(taxa_presenca__gte=80)
            .count(),
            ...
            .values("mes")
            .annotate(
                presentes=Count("id", filter=Q(presente=True)), total=Count("id")
            ),
        }
        ...
        self.assertLess(
            tempo_calculo, 2.0, f"Cálculo de estatísticas muito lento: {tempo_calculo}s"
        )
...
class MemoryUsageTest(PerformanceTestCase):
    ...
    def test_memoria_consulta_grande_dataset(self):
        ...
        presencas_queryset = (
            PresencaAcademica.objects.filter(turma=self.turma)
