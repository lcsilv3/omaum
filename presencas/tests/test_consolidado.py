...
class ConsolidadoPresencasViewTestCase(TestCase):
    ...
    def test_filtros_consolidado_post(self):
        ...
        response = self.client.post(self.filtros_url, {
            'turma_id': self.turma.id,
            'periodo_inicio': '2024-01-01',
            'periodo_fim': '2024-12-31'
        })
...
class ConsolidadoIntegrationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.curso = Curso.objects.create(nome='Curso Integração', codigo='CI001', ativo=True)
        self.turma = Turma.objects.create(nome='Turma Integração', curso=self.curso, status='A')
