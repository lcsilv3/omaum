...
def consolidado_aluno(request, aluno_id):
    ...
    try:
        ...
        # Retornar resposta
        if formato == 'html':
            return render(request, 'presencas/consolidado_aluno.html', {
...
def tabela_consolidada(request):
    ...
    try:
        ...
        if formato == 'html':
            ...
            return render(request, 'presencas/tabela_consolidada.html', {
                'tabela': tabela,
                'turma': turma,
                'atividade': atividade,
                'filtros': {
...
def estatisticas_turma(request, turma_id):
    ...
    try:
        ...
        # Retornar resposta
        if formato == 'html':
            return render(request, 'presencas/estatisticas_turma.html', {
        ...
        logger.error(f"Erro de validação nas estatísticas da turma {turma_id}: {str(e)}")
        return JsonResponse({'erro': str(e)}, status=400)
    except Exception as e:
...
def dashboard_presencas(request):
    ...
    try:
        ...
        return render(request, 'presencas/dashboard.html', {
...
def exportar_consolidado(request):
    ...
    try:
        ...
        # Cabeçalho
        writer.writerow([
            'Aluno', 'CPF', 'Turma', 'Convocações', 'Presenças', 'Faltas',
            'Percentual', 'Voluntários Extra', 'Voluntários Simples',
            'Total Voluntários', 'Carências', 'Status'
        ])
        ...
        writer.writerow(['Total de Alunos', tabela['estatisticas_gerais']['total_alunos']])
        writer.writerow(['Percentual Médio', f"{tabela['estatisticas_gerais']['percentual_medio']:.2f}%"])
        writer.writerow(['Total Convocações', tabela['estatisticas_gerais']['total_convocacoes']])
        writer.writerow(['Total Presenças', tabela['estatisticas_gerais']['total_presencas']])
        writer.writerow(['Total Carências', tabela['estatisticas_gerais']['total_carencias']])
