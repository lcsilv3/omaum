...
class PresencaDetalhada(models.Model):
    ...
    faltas = models.PositiveIntegerField(
        default=0, verbose_name="Faltas (F)", help_text="Número de faltas no período"
    )
    ...
    total_voluntarios = models.PositiveIntegerField(
        default=0, verbose_name="Total de Voluntários", help_text="V1 + V2"
    )
    ...
    def __str__(self):
        return (
            f"{self.aluno.nome} - {self.periodo.strftime('%m/%Y')} - {self.atividade}"
        )
    ...
    def calcular_percentual(self):
        ...
        percentual = (Decimal(self.presencas) / Decimal(self.convocacoes)) * Decimal(
            "100"
        )
    ...
    def calcular_carencias(self):
        ...
        try:
            configuracao = ConfiguracaoPresenca.objects.get(
                turma=self.turma, atividade=self.atividade, ativo=True
            )
...
class ConfiguracaoPresenca(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        logger.info(f"Configuração de presença salva: {self.turma} - {self.atividade}")
...
class AgendamentoRelatorio(models.Model):
    ...
    titulo_personalizado = models.CharField(
        max_length=500, blank=True, verbose_name="Título Personalizado"
    )

    # Filtros
    periodo = models.CharField(
        max_length=20, choices=PERIODO_CHOICES, default="atual", verbose_name="Período"
    )
    ...
    incluir_graficos = models.BooleanField(
        default=True, verbose_name="Incluir Gráficos"
    )
    ...
    emails_destino = models.TextField(
        verbose_name="Emails de Destino", help_text="Emails separados por vírgula"
    )
    ...
    def clean(self):
        ...
        if self.frequencia == "semanal" and self.dia_semana is None:
            raise ValidationError("Dia da semana é obrigatório para frequência semanal")

        if (
            self.frequencia in ["mensal", "trimestral", "semestral", "anual"]
            and self.dia_mes is None
        ):
            raise ValidationError(
    ...
    def calcular_proxima_execucao(self):
        ...
        if self.frequencia == "diario":
            ...
            self.proxima_execucao = base_date.replace(
                year=target_year, month=target_month, day=min(self.dia_mes, 28)
            )
            ...
            self.proxima_execucao = base_date.replace(
                year=target_year, month=self.dia_mes, day=min(self.dia_mes, 28)
            )
