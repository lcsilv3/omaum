...
class APIResponseMixin:
    ...
    def success_response(self, data=None, message="Operação realizada com sucesso"):
        ...
        response = {"success": True, "message": message, "data": data or {}}
        ...
        try:
            ...
            if not presencas_data:
                return self.error_response(
                    "Nenhuma presença fornecida para atualização"
                )
            ...
            if erros_validacao:
                return self.error_response("Dados inválidos", erros_validacao)
                ...
                for presenca_data in presencas_data:
                    try:
                        ...
                        if not created:
                            ...
                            presenca.convocacoes = presenca_data.get(
                                "convocacoes", presenca.convocacoes
                            )
                            presenca.presencas = presenca_data.get(
                                "presencas", presenca.presencas
                            )
                            presenca.faltas = presenca_data.get(
                                "faltas", presenca.faltas
                            )
                            presenca.voluntario_extra = presenca_data.get(
                                "voluntario_extra", presenca.voluntario_extra
                            )
                            presenca.voluntario_simples = presenca_data.get(
                                "voluntario_simples", presenca.voluntario_simples
                            )
                            presenca.save()
                        ...
                        logger.error(
                            f"Erro ao processar presença {presenca_data}: {str(e)}"
                        )
                        return self.error_response(
                            f"Erro ao processar presença: {str(e)}"
                        )
...
class CalcularEstatisticasView(APIResponseMixin, View):
    ...
    def get(self, request):
        ...
        try:
            ...
            stats_por_aluno = (
                queryset.values("aluno__nome", "aluno__pk")
                .annotate(
                    total_convocacoes=Sum("convocacoes"),
            ...
            if not turma_id:
                stats_por_turma = (
                    queryset.values("turma__nome", "turma__id")
                    .annotate(
                        total_convocacoes=Sum("convocacoes"),
                        total_presencas=Sum("presencas"),
                        total_faltas=Sum("faltas"),
            ...
            if not atividade_id:
                stats_por_atividade = (
                    queryset.values("atividade__nome", "atividade__id")
                    .annotate(
                        total_convocacoes=Sum("convocacoes"),
                        total_presencas=Sum("presencas"),
                        total_faltas=Sum("faltas"),
    ...
    def get(self, request):
        ...
        try:
            ...
            if turma_id:
                ...
                matriculas = Matricula.objects.filter(turma_id=turma_id).values_list(
                    "aluno_id", flat=True
                )
            ...
            if turma_id:
                turma_map = {
                    m.aluno_id: m.turma
                    for m in Matricula.objects.filter(turma_id=turma_id)
                }
            for aluno in alunos:
                turma_info = None
        ...
        try:
            ...
            if all(campo in data for campo in ["convocacoes", "presencas", "faltas"]):
                ...
                if convocacoes > 0:
                    ...
                    if percentual < 50:
                        warnings.append(
                            f"Percentual de presença baixo: {percentual:.1f}%"
                        )
            ...
            if not erros and "aluno_id" in data:
                ...
                if existe_registro:
                    warnings.append(
                        "Já existe registro para este aluno/turma/atividade/período"
                    )
            ...
            return self.success_response(
                {"valido": True, "warnings": warnings, "dados_validados": data}
            )
            ...
            for atividade in atividades:
                ...
                try:
                    configuracao = ConfiguracaoPresenca.objects.get(
                        turma=turma, atividade=atividade, ativo=True
                    )
                ...
                atividades_data.append(
                    {
                        ...
                        "nome": atividade.nome,
                        "tipo": atividade.tipo
                        if hasattr(atividade, "tipo")
                        else "Geral",
                        "obrigatoria": configuracao.obrigatoria
                        if configuracao
                        else True,
                        "peso_calculo": float(configuracao.peso_calculo)
                        if configuracao
                        else 1.0,
        ...
        try:
            ...
            for config in configuracoes:
                config_data.append(
                    {
                        "id": config.id,
                        "turma": {"id": config.turma.id, "nome": config.turma.nome},
                        "atividade": {
            ...
            return self.success_response(
                {"configuracoes": config_data, "total": len(config_data)}
            )
