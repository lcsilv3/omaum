...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--verbose", action="store_true", help="Saída verbose com detalhes"
        )

    def handle(self, *args, **options):
        """Executa as verificações de saúde."""

        self.stdout.write(self.style.SUCCESS("🏥 OMAUM System Health Check"))
        ...
        for check in checks_to_run:
            ...
            try:
                ...
                self.stdout.write(self.style.ERROR(f"❌ ERRO {check.upper()}: {e}"))
        ...
        try:
            ...
            return {
                ...
                "details": {
                    ...
                    if "total_active_tasks" in locals()
                    else 0,
                    "issues": issues,
                },
            }
            ...
            if metrics:
                ...
                if total_requests > 0:
                    ...
                    if slow_percentage > 10:  # Mais de 10% de requests lentas
                        issues.append(
                            f"Alto percentual de requests lentas: {slow_percentage:.1f}%"
                        )
    ...
    def check_security(self, options: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                or settings.SECRET_KEY == "your-production-secret-key-here"
            ):
    ...
    def generate_report(self, options: Dict[str, Any], total_time: float):
        ...
        # Contar status
        healthy_count = sum(
            1 for r in self.results.values() if r.get("status") == "healthy"
        )
        warning_count = sum(
            1 for r in self.results.values() if r.get("status") == "warning"
        )
        error_count = sum(
