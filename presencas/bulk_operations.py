...
class BulkPresencaOperations:
    ...
    @staticmethod
    @transaction.atomic
    def criar_presencas_lote(dados_presencas: List[Dict[str, Any]], registrado_por: str) -> Dict[str, int]:
        """
        ...
        for i, dados in enumerate(dados_presencas):
            required_fields = ['aluno_id', 'turma_id', 'atividade_id', 'data', 'presente']
            missing_fields = [field for field in required_fields if field not in dados]
            if missing_fields:
                raise ValidationError(f"Item {i}: Campos obrigatórios ausentes: {missing_fields}")
        ...
        for dados in dados_presencas:
            ...
            if dados['atividade_id'] not in atividades:
                raise ValidationError(f"Atividade ID {dados['atividade_id']} não encontrada")
        
        # Verificar presenças existentes para evitar duplicatas
        existing_keys = set()
        for dados in dados_presencas:
            key = (dados['aluno_id'], dados['turma_id'], dados['atividade_id'], dados['data'])
    ...
    @staticmethod
    def otimizar_queries_estatisticas(turma_id: int = None, periodo_inicio: date = None, periodo_fim: date = None) -> Dict[str, Any]:
        """
        ...
        stats = queryset.aggregate(
            ...
            total_presentes=Count(Case(When(presente=True, then=1), output_field=IntegerField())),
            total_ausentes=Count(Case(When(presente=False, then=1), output_field=IntegerField())),
            ...
            percentual_presenca=Avg(Case(When(presente=True, then=100), default=0, output_field=IntegerField()))
        )
        ...
        stats['por_aluno'] = list(
            ...
            .annotate(
                ...
                ausentes=Count(Case(When(presente=False, then=1), output_field=IntegerField()))
            )
            .order_by('aluno__nome')
        )
        
        logger.info(f"Estatísticas calculadas: {stats['total_registros']} registros processados")
        return stats
