...
class RegistrarPresencaForm(forms.Form):
    ...
    ANO_CHOICES = [
        (ano, str(ano))
        for ano in range(timezone.now().year, timezone.now().year - 5, -1)
    ]
    MES_CHOICES = [
        (1, "Janeiro"),
        (2, "Fevereiro"),
    ...
    ano = forms.ChoiceField(
        choices=ANO_CHOICES, label="Ano", required=True, initial=timezone.now().year
    )
    mes = forms.ChoiceField(
        choices=MES_CHOICES, label="Mês", required=True, initial=timezone.now().month
    )
            ...
            for aluno in alunos:
                self.fields[f"aluno_{aluno.cpf}_status"] = forms.ChoiceField(
                    ...
                    widget=forms.RadioSelect(
                        attrs={"class": "form-check-input", "aria-label": aluno.nome}
                    ),
                )
                self.fields[f"aluno_{aluno.cpf}_justificativa"] = forms.CharField(
                    ...
                    widget=forms.TextInput(
                        attrs={
                            ...
                            "class": "form-control",
                            "aria-label": f"Justificativa {aluno.nome}",
                        }
                    ),
                )
...
class FiltroConsolidadoForm(forms.Form):
    ...
    aluno_nome = forms.CharField(
        label="Nome do Aluno",
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "Digite o nome do aluno"}
        ),
        required=False,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "turma" in self.data:
            try:
                ...
                self.fields["atividade"].queryset = (
                    AtividadeAcademica.objects.filter(
...
class EditarPresencaDetalhadaForm(forms.Form):
    ...
    convocacoes = forms.IntegerField(
        ...
        max_value=999,
        widget=forms.NumberInput(
            attrs={"class": "form-control form-control-sm", "style": "width: 60px;"}
        ),
    )
