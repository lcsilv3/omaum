...
class CargoViewTest(TestCase):
    ...
    def test_listar_cargos(self):
        response = self.client.get(
            reverse("cargos:listar_cargos_administrativos")
        )
    ...
    def test_detalhe_cargo(self):
        response = self.client.get(
            reverse("cargos:detalhe_cargo", args=[self.cargo.codigo_cargo])
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Coordenador")
    ...
    def test_editar_cargo(self):
        ...
        self.assertEqual(
            cargo.descricao, "Coordenador com experiência avançada."
        )
