...
class AlignedIndentFilter:
    ...
    def nl(self, offset=1):
        ...
        return sql.Token(
            T.Whitespace,
            self.n
            + self.char * (self._max_kwd_len + offset + indent + self.offset),
        )

    def _process_statement(self, tlist):
        if (
            len(tlist.tokens) > 0
            and tlist.tokens[0].is_whitespace
            and self.indent == 0
        ):
            tlist.tokens.pop(0)
    ...
    def _process_case(self, tlist):
        ...
        condition_width = [
            len(" ".join(map(str, cond))) if cond else 0 for cond, _ in cases
        ]
    ...
    def _split_kwds(self, tlist):
        ...
        while token:
            ...
            # word as aligner
            if token.match(
                T.Keyword, self.join_words, regex=True
