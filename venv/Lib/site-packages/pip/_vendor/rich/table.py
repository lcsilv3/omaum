...
class Table(JupyterMixin):
    ...
    def get_row_style(self, console: "Console", index: int) -> StyleType:
        ...
        if self.row_styles:
            style += console.get_style(
                self.row_styles[index % len(self.row_styles)]
            )
    ...
    def __rich_measure__(
        ...
        minimum_width = (
            sum(measurement.minimum for measurement in measurements)
            + extra_width
        )
        maximum_width = (
            sum(measurement.maximum for measurement in measurements)
            + extra_width
    ...
    def _calculate_column_widths(
        ...
        columns = self.columns
        width_ranges = [
            self._measure_column(console, options, column)
        ...
        if self.expand:
            ...
            if any(ratios):
                ...
                flex_widths = ratio_distribute(
                    flexible_width, ratios, flex_minimum
                )
                iter_flex_widths = iter(flex_widths)
                for index, column in enumerate(columns):
                    if column.flexible:
                        widths[index] = fixed_widths[index] + next(
                            iter_flex_widths
                        )
        ...
        if table_width > max_width:
            widths = self._collapse_widths(
                widths,
                [
                    (column.width is None and not column.no_wrap)
                    for column in columns
                ],
                max_width,
            )
            ...
            if table_width > max_width:
                ...
                widths = ratio_reduce(
                    excess_width, [1] * len(widths), widths, widths
                )
                table_width = sum(widths)

            width_ranges = [
                self._measure_column(
                    console, options.update_width(width), column
                )
        ...
        if (table_width < max_width and self.expand) or (
            ...
            and table_width < (self.min_width - extra_width)
        ):
            _max_width = (
                max_width
        ...
        if any(wrapable):
            while total_width and excess_width > 0:
                ...
                if not any(ratios) or not column_difference:
                    break
                max_reduce = [min(excess_width, column_difference)] * len(
                    widths
                )
    ...
    def _measure_column(
        ...
        measurement = measurement.clamp(
            ...
            else column.min_width + padding_width,
            None
            if column.max_width is None
            else column.max_width + padding_width,
        )
        return measurement
    ...
    def _render(
        ...
        _box = (
            ...
            if _box and not self.show_header
            else _box
        )
        ...
        for index, (first, last, row_cell) in enumerate(
            ...
            def align_cell(
                ...
                if vertical == "top":
                    ...
                    return _Segment.align_middle(
                        cell, width, row_height, style
                    )
