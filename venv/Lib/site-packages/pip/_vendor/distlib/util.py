...
def parse_requirement(req):
    ...
    if not versions:
        ...
        rs = "%s %s" % (
            distname,
            ", ".join(["%s %s" % con for con in versions]),
        )
    return Container(
        name=distname,
        extras=extras,
    ...
    def newer(self, source, target):
        ...
        if not os.path.exists(source):
            raise DistlibException(
                "file '%r' does not exist" % os.path.abspath(source)
            )
    ...
    def byte_compile(
        ...
        if not self.dry_run:
            ...
            if hashed_invalidation and hasattr(
                ...
                if not isinstance(
                    hashed_invalidation, py_compile.PycInvalidationMode
                ):
                    hashed_invalidation = (
...
def get_export_entry(specification):
    ...
    if not m:
        ...
        if "[" in specification or "]" in specification:
            raise DistlibException(
                "Invalid specification " "'%s'" % specification
            )
    else:
        d = m.groupdict()
        name = d["name"]
...
class Sequencer(object):
    ...
    def is_step(self, step):
        return (
            step in self._preds or step in self._succs or step in self._nodes
        )
