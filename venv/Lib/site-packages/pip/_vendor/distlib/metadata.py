...
def _version2fieldlist(version):
    if version == "1.0":
        ...
        return _345_FIELDS + tuple(
            f for f in _566_FIELDS if f not in _345_FIELDS
        )
    elif version == "2.0":
...
class Metadata(object):
        ...
        if mapping is not None:
            ...
            if data is None:
                ...
                try:
                    ...
                    # that to propagate
                    self._legacy = LegacyMetadata(
                        fileobj=StringIO(data), scheme=scheme
                    )
                    self.validate()
        ...
        if self._legacy:
            ...
            for key in ("build", "dev", "test"):
                ...
                if e in extras:
                    ...
                    result.extend(
                        self.get_requirements(reqts, extras=extras, env=env)
                    )
        return result
    ...
    def validate(self):
        if self._legacy:
            ...
            if missing or warnings:
                logger.warning(
                    "Metadata: missing: %s, warnings: %s", missing, warnings
                )
        else:
    ...
    def write(self, path=None, fileobj=None, legacy=False, skip_unknown=True):
        ...
        if legacy:
            ...
            if fileobj:
                json.dump(
                    d, fileobj, ensure_ascii=True, indent=2, sort_keys=True
                )
            else:
                with codecs.open(path, "w", "utf-8") as f:
                    json.dump(
                        d, f, ensure_ascii=True, indent=2, sort_keys=True
                    )
