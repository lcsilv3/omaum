...
class Flags:
    ...
    def is_(self, key: Key, flag: int) -> bool:
        ...
        if key_stem in cont:
            ...
            return (
                flag in inner_cont["flags"]
                or flag in inner_cont["recursive_flags"]
            )
        return False
...
def skip_until(
    ...
    *,
    ...
    try:
        ...
        if error_on_eof:
            raise TOMLDecodeError(
                f"Expected {expect!r}", src, new_pos
            ) from None
    ...
    if not error_on.isdisjoint(src[pos:new_pos]):
        while src[pos] not in error_on:
            pos += 1
        raise TOMLDecodeError(
            f"Found invalid character {src[pos]!r}", src, pos
        )
    return new_pos
...
def create_list_rule(src: str, pos: Pos, out: Output) -> tuple[Pos, Key]:
    ...
    if out.flags.is_(key, Flags.FROZEN):
        raise TOMLDecodeError(
            f"Cannot mutate immutable namespace {key}", src, pos
        )
...
def key_value_rule(
    ...
    for cont_key in relative_path_cont_keys:
        ...
        if out.flags.is_(cont_key, Flags.EXPLICIT_NEST):
            raise TOMLDecodeError(
                f"Cannot redefine namespace {cont_key}", src, pos
            )
...
def parse_inline_table(
    ...
    while True:
        ...
        if key_stem in nest:
            raise TOMLDecodeError(
                f"Duplicate inline table key {key_stem!r}", src, pos
            )
        nest[key_stem] = value
        pos = skip_chars(src, pos, TOML_WS)
        c = src[pos : pos + 1]
