...
if sys.version_info >= (3, 13, 0, "beta"):
    ...
    class _SpecialGenericAlias(_special_generic_alias_base, _root=True):
        def __init__(
            self, origin, nparams, *, inst=True, name=None, defaults=()
        ):
            if _special_generic_alias_base is typing._GenericAlias:
                ...
                super().__init__(
                    origin, nparams, special=True, inst=inst, name=name
                )
            else:
                # Python >= 3.9
        ...
        def __getitem__(self, params):
            ...
            if (
                ...
                params = (
                    *params,
                    *self._defaults[len(params) - self._nparams :],
                )
            ...
            if actual_len != self._nparams:
                if self._defaults:
                    expected = (
                        f"at least {self._nparams - len(self._defaults)}"
                    )
                else:
    ...
    class _ProtocolMeta(type(typing.Protocol)):
        ...
        def __new__(mcls, name, bases, namespace, **kwargs):
            if name == "Protocol" and len(bases) < 2:
                ...
                for base in bases:
                    if not (
                        base
                        in {object, typing.Generic, Protocol, typing.Protocol}
                        or base.__name__
        ...
        def __instancecheck__(cls, instance):
            ...
            for attr in cls.__protocol_attrs__:
                ...
                if (
                    val is None
                    and attr not in cls.__non_callable_proto_members__
                ):
                    break
            else:
                return True
...
if _PEP_728_IMPLEMENTED:
    ...
    class _TypedDictMeta(type):
        def __new__(cls, name, bases, ns, *, total=True, closed=False):
            ...
            for base in bases:
                if (
                    type(base) is not _TypedDictMeta
                    and base is not typing.Generic
                ):
                    raise TypeError(
    ...
    def TypedDict(
        typename, fields=_marker, /, *, total=True, closed=False, **kwargs
    ):
...
if hasattr(typing, "ReadOnly"):  # 3.13+
    ...
    def _strip_extras(t):
        ...
        if hasattr(_types, "GenericAlias") and isinstance(
            t, _types.GenericAlias
        ):
            stripped_args = tuple(_strip_extras(a) for a in t.__args__)
            if stripped_args == t.__args__:
                return t
    ...
    def get_type_hints(obj, globalns=None, localns=None, include_extras=False):
        ...
        if hasattr(typing, "Annotated"):  # 3.9+
            ...
            hint = typing.get_type_hints(
                obj, globalns=globalns, localns=localns
            )
        if include_extras:
            return hint
        ...
        def __new__(
            ...
            *constraints,
            ...
            def _tvar_prepare_subst(alias, args):
                if typevar.has_default() and alias.__parameters__.index(
                    typevar
                ) == len(args):
    ...
    class ParamSpec(list, _DefaultMixin):
        ...
        def __init__(
            ...
            *,
            ...
            if bound:
                self.__bound__ = typing._type_check(
                    bound, "Bound must be a type."
                )
            else:
                self.__bound__ = None
...
# 3.9
    ...
    def TypeGuard(self, parameters):
        ...
        item = typing._type_check(
            parameters, f"{self} accepts only a single type."
        )
        ...
        def __typing_unpacked_tuple_args__(self):
            ...
            if isinstance(arg, (typing._GenericAlias, _types.GenericAlias)):
                if arg.__origin__ is not tuple:
                    raise TypeError(
                        "Unpack[...] must be used with a tuple type"
                    )
                return arg.__args__
            return None
        ...
        def __new__(cls, name, *, default=NoDefault):
            ...
            def _typevartuple_prepare_subst(alias, args):
                ...
                for k, arg in enumerate(args):
                    if not isinstance(arg, type):
                        subargs = getattr(
                            arg, "__typing_unpacked_tuple_args__", None
                        )
                        if (
                            subargs
                            and len(subargs) == 2
                ...
                return (
                    ...
                    *(
                        [fillarg]
                        * (plen - right - left - typevartuple_index - 1)
                    ),
                    *args[alen - right :],
                )
    ...
    def dataclass_transform(
        *,
        ...
        field_specifiers: typing.Tuple[
            typing.Union[
                typing.Type[typing.Any], typing.Callable[..., typing.Any]
            ],
            ...,
        ] = (),
        **kwargs: typing.Any,
    ...
    def _check_generic(cls, parameters, elen=_marker):
        ...
        if alen != elen:
            ...
            if hasattr(cls, "__parameters__"):
                ...
                num_tv_tuples = sum(
                    isinstance(p, TypeVarTuple) for p in parameters
                )
            ...
            things = (
                "arguments" if sys.version_info >= (3, 10) else "parameters"
            )
            raise TypeError(
...
def _has_generic_or_protocol_as_origin() -> bool:
    ...
    #   and the call stack isn't deep enough
        ...
        # throws an error.
        return (
            origin is typing.Generic
            or origin is Protocol
            or origin is typing.Protocol
        )
...
def _is_unpacked_typevartuple(x) -> bool:
    ...
    args = get_args(x)
    return (
        bool(args) and len(args) == 1 and type(args[0]) in _TYPEVARTUPLE_TYPES
    )
        ...
        for t in args:
            if isinstance(t, type):
                ...
                if t not in parameters:
                    if enforce_default_ordering:
                        has_default = (
                            getattr(t, "__default__", NoDefault)
                            is not NoDefault
                        )
...
if sys.version_info >= (3, 13):
    ...
    def _make_nmtuple(name, types, module, defaults=()):
        ...
        nm_tpl = collections.namedtuple(
            name, fields, defaults=defaults, module=module
        )
    ...
    class _NamedTupleMeta(type):
        def __new__(cls, typename, bases, ns):
            ...
            bases = tuple(
                tuple if base is _NamedTuple else base for base in bases
            )
            if "__annotations__" in ns:
